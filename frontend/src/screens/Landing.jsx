    ...
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, alignItems: "stretch" }} className="landing-hero">
        ...
        <div style={{ ...card, background: "#1a1c22", border: "1px solid #1a1c22", color: "#fff" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <Badge color="#e5e7ef">{t(`${tokensMonat} Tokens jeden Monat – rund 35 bis 40 Aufgaben`, `${tokensMonat} tokens every month – around 35 to 40 tasks`)}</Badge>
...
export default function Landing() {
  ...
  const FAQ = [
    ...
    { q: t("Was kostet Kniff?", "What does Kniff cost?"),
      a: t(`Die ersten ${probe} Aufgaben sind geschenkt – ohne Zahlungsangaben. Danach kostet ${plusName} CHF ${monat} im Monat oder ${jahr}.– im Jahr pro Kind. Darin sind jeden Monat ${tokensMonat} Tokens enthalten, das reicht für rund 35 bis 40 Aufgaben; wer mehr braucht, kauft ein Paket dazu. Jederzeit kündbar, das Abo läuft dann bis zum Ende der bezahlten Zeit.`,
           `The first ${probe} tasks are on us – no payment details. After that ${plusName} costs CHF ${monat} a month or ${jahr}.– a year per child. That includes ${tokensMonat} tokens every month, enough for around 35 to 40 tasks; if you need more you buy a package. Cancel anytime; the subscription then runs until the end of the paid period.`) },
    { q: t("Was ist ein Token?", "What is a token?"),
      a: t(`Ein Token ist ein Rappen KI-Leistung. Jede Antwort von Kniff kostet je nach Aufgabe ein bis vier Tokens, eine Foto-Erkennung etwa zwei – eine ganze Aufgabe mit allen Zwischenschritten im Schnitt ${TOKENS_PRO_AUFGABE}. Die Abo-Tokens kommen jeden Monat neu, was übrig bleibt, verfällt; nachgekaufte Tokens verfallen nie.`,
           `A token is one Rappen of AI computation. Each answer from Kniff costs one to four tokens depending on the task, a photo recognition about two – a whole task with all steps about ${TOKENS_PRO_AUFGABE} on average. Subscription tokens renew every month and whatever is left expires; tokens you top up never expire.`) },
